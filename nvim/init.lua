  ...
  require("packer").startup({
    function(use)
      ...
      use({
        ...
        requires = { "nvim-telescope/telescope-file-browser.nvim", "hrsh7th/nvim-cmp" },
        config = function()
          ...
          local function table_keys_to_string(tbl, delimiter)
            local keys = {}
            for key, _ in pairs(tbl) do
              table.insert(keys, key)
            end
            return table.concat(keys, delimiter)
          end

          local function cmp_tab(prompt_bufnr)
            if cmp.visible() then
          ...
          local function cmp_s_tab(prompt_bufnr)
            if cmp.visible() then
              cmp.select_previous_item({ behavior = cmp.SelectBehavior })
            else
              actions.toggle_selection(prompt_bufnr)
          ...
          local function cmp_cr(prompt_bufnr)
            if cmp.visible() then
              cmp.confirm()
            else
          ...
          vim.keymap.set("n", "[telescope]/", function()
            builtin.buffers(themes.get_ivy())
          end, { noremap = true })
          vim.keymap.set("n", "[telescope]f", function()
            builtin.live_grep(themes.get_ivy())
          end, { noremap = true })
          vim.keymap.set("n", "[telescope]y", function()
            builtin.registers(themes.get_ivy())
          end, { noremap = true })
          vim.keymap.set("n", "[telescope]q", function()
      ...
      use({
        ...
        function! SlimeConfirm() abort
          ...
          vim.api.nvim_set_keymap(
            "v",
            "[slime]l",
            ":<c-u>call slime#send_op(visualmode(), 1) | call SlimeConfirm()<cr>",
            {}
          )
          ...
          vim.api.nvim_set_keymap(
            "",
            "[slime]b",
            ':call slime#send_range(line(1), line("$")) | call SlimeConfirm()<cr>',
            {}
          )
          vim.cmd([[
      ...
      use({
        ...
        config = function()
          ...
          vim.cmd(
            [[ command! -nargs=1 -complete=customlist,v:lua.cmp_sources_list CmpDisable lua disable_cmp_source(<f-args>) ]]
          )
          vim.cmd(
            [[ command! -nargs=1 -complete=customlist,v:lua.cmp_sources_list CmpEnable lua enable_cmp_source(<f-args>) ]]
          )
        end,
      })
      ---}}}
      ...
      use({
        ...
        config = function()
          ...
          local function resize(img)
            os.execute(string.format('mogrify -quality 95 -resize "600x400>" "%s"', img.path))
          end
          ...
          vim.api.nvim_create_user_command("PastePNG", function(opts)
            require("clipboard-image.paste").paste_img({ img_format = "png" })
          end, { nargs = 0 })
          vim.api.nvim_create_user_command("PasteJPG", function(opts)
            require("clipboard-image.paste").paste_img({ img_format = "jpg" })
          end, { nargs = 0 })
          vim.api.nvim_create_user_command("PasteWEBP", function(opts)
            require("clipboard-image.paste").paste_img({ img_format = "webp" })
          end, { nargs = 0 })
        end,
      })
      --}}}
...
_G.load_config = function()
  ...
  vim.api.nvim_set_keymap(
    "v",
    "<s-down>",
    "<esc>:set fdm=manual<cr>'<V'>:m '>+1<cr>:set fdm=marker<cr>gv",
    { noremap = true }
  )
  vim.api.nvim_set_keymap(
    "v",
    "<s-up>",
    "<esc>:set fdm=manual<cr>'<V'>:m '<-2<cr>:set fdm=marker<cr>gv",
    { noremap = true }
  )
  ---}}}
  ...
  vim.api.nvim_set_keymap(
    "n",
    "<leader>dt",
    ':execute "silent normal! mq" \':%s/\\s\\+$//ge\' "\\r`q"<cr>',
    { silent = true }
  )
  ---}}}
